...
from parser import recursive_json_to_text
            ...
            for category, content in raw_data.items():
                if isinstance(content, list):
                    for item in content:
                        ...
                        doc_title = item.get(
                            "PolicyType", item.get("question", "General Info")
                        )
                        documents.append(
                            Document(
                    ...
                    documents.append(
                        Document(
                            page_content=flat_text, metadata={"category": category}
                        )
                    )

        print(
            f"Constructed {len(documents)} dynamic documents. Persisting to ChromaDB..."
        )
