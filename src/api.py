...
class ChatRequest(BaseModel):
    session_id: str = Field(..., description="Unique ID to maintain chat context across turns.")
    message: str = Field(..., description="The user's input message.")

class ChatResponse(BaseModel):
    response: str
    processing_time_ms: float = Field(..., description="Time taken to generate the response in milliseconds.")
    timestamp: str = Field(..., description="UTC timestamp of the response.")

# 2. Optimized Standard Chat Endpoint (Async)
...
# 3. Streaming Chat Endpoint (Best for UX)
@chat_router.post("/chat/stream")
async def chat_stream_endpoint(request: ChatRequest):
    """Streams the agent's response token-by-token for blazing fast UX."""
    logger.info(f"Received streaming request | Session: {request.session_id} | Message: '{request.message}'")

    async def event_stream():
        config = {"configurable": {"thread_id": request.session_id}}

        try:
            # astream_events gives us granular control over every token and tool call
            async for event in agent_app.astream_events(
                {"messages": [("user", request.message)]},
                config=config,
                version="v2"
            ):
                # We only want to stream the tokens from the final LLM response, not the tool calls
                if event["event"] == "on_chat_model_stream":
                    chunk = event["data"]["chunk"]
                    if chunk.content:
                        # Yield the token in Server-Sent Events (SSE) format
                        yield f"data: {json.dumps({'token': chunk.content})}\n\n"

        except Exception as e:
            logger.error(f"Streaming error for session '{request.session_id}': {str(e)}", exc_info=True)
